...
def _tensor_is_xpu(t: torch.Tensor) -> bool:
    return bool(getattr(t, "is_xpu", False))
...
class Generator(nn.Module):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        if (input.is_cuda or _tensor_is_xpu(input)) and self.ngpu > 1:
            return nn.parallel.data_parallel(self.main, input, range(self.ngpu))
        return self.main(input)


class Discriminator(nn.Module):
    def __init__(self, nc: int, ndf: int, ngpu: int):
...
def _default_workers() -> int:
    # On Windows, num_workers>0 requires guarding the entrypoint.
    # We still add a guard below, but default to 0 to be beginner-friendly.
...
def main() -> None:
    ...
    parser.add_argument('--workers', type=int, help='number of data loading workers', default=_default_workers())
    ...
    parser.add_argument(
        '--device',
        default=None,
        help='training device: cpu | cuda | xpu | auto (default: cpu; or auto if --accel is set)',
    )
    parser.add_argument('--accel', action='store_true', default=False, help='(legacy) enables auto device selection')
    ...
    requested_device = (opt.device or ('auto' if opt.accel else 'cpu')).lower()

    if requested_device == 'cpu':
        device = torch.device('cpu')
    elif requested_device == 'cuda':
        if not torch.cuda.is_available():
            raise RuntimeError('CUDA was requested but is not available. Install a CUDA-enabled PyTorch build and GPU drivers.')
        device = torch.device('cuda:0')
    elif requested_device == 'xpu':
        if not (hasattr(torch, 'xpu') and torch.xpu.is_available()):
            raise RuntimeError('XPU was requested but is not available in this PyTorch build / system.')
        ...
        raise ValueError(f"Unknown --device value: {opt.device}")
    ...
    if opt.dataset in ['imagenet', 'folder', 'lfw']:
        ...
        if _dir_has_subdirs(opt.dataroot):
            dataset = dset.ImageFolder(root=opt.dataroot, transform=tfm)
        else:
            dataset = FlatFolderDataset(root=opt.dataroot, transform=tfm)
        nc = 3
    ...
    if opt.progress and tqdm is None:
        raise RuntimeError('Progress bar requested but tqdm is not installed. Install it with: pip install tqdm')

    for epoch in range(opt.niter):
        data_iter = enumerate(dataloader, 0)
        if opt.progress:
            data_iter = tqdm(data_iter, total=len(dataloader), desc=f"Epoch {epoch + 1}/{opt.niter}")
